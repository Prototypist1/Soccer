    ...
    class AITeam
    {
        ...
        private void Update()
        {
            ...
            else if (gameState.GameBall.OwnerOrNull is Guid owner)
            {
                if (team.TryGetValue(owner, out var hasBall)) // we have the ball
                {
...

                    {
                        ...
                        {
                            if (hasBallGenerators.First().generator(gameState).Length == 0)
                            {
                                var db = 0;
                            }
            ...
            foreach (var member in inputs)
            {
                ...
                {
                    var (move, boost) = GenerateDirectionFoot(gameState.players[member.Key].PlayerFoot.Position, gameState.players[member.Key].PlayerBody.Position, member.Key);
